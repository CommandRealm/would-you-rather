...
execute as @e[tag=rab_lookout,tag=rab_lookout_2,type=zombie] at @s at @s[z=-400,distance=..1] run tag @s add rab_move_z
...
execute if score $current rab_money >= $goal rab_money unless entity @e[type=shulker,tag=rab_glowbox] run summon shulker 15991 67 -376 {Tags:["rab_die","rab_glowbox"],Glowing:1b,Invulnerable:1b,ActiveEffects:[{Id:14,Amplifier:1b,Duration:20000000,ShowParticles:0b}],NoAI:1b,Silent:1b}
