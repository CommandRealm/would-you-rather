...
execute if score $number skip_coop matches 0 run setblock -938 79 -130 oak_wall_sign[facing=north]{front_text:{messages:['{"text":"Skip pointless","color":"red","bold":true,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 1.. run function options:error"}}', '{"text":"scenarios:","color":"red","bold":true}', '{"text":"Off","color":"red"}', '{"text":"<<<         >>>","color":"aqua","bold":true,"clickEvent":{"action":"run_command","value":"/execute as @s at @s anchored eyes if score $game state matches 0 run function options:co-op/click_co-op"}}'], has_glowing_text: false, color: "black"}}
execute if score $number skip_coop matches 1 run setblock -938 79 -130 oak_wall_sign[facing=north]{front_text:{messages:['{"text":"Skip pointless","color":"red","bold":true,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 1.. run function options:error"}}', '{"text":"scenarios:","color":"red","bold":true}', '{"text":"On","color":"green"}', '{"text":"<<<         >>>","color":"aqua","bold":true,"clickEvent":{"action":"run_command","value":"/execute as @s at @s anchored eyes if score $game state matches 0 run function options:co-op/click_co-op"}}'], has_glowing_text: false, color: "black"}}
